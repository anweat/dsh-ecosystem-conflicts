 ...
 *
 * Usage:
 *   node arbitration/tokens-cli.mjs emit <dsh-root> [out.d.ts]   write the declaration
 *   node arbitration/tokens-cli.mjs lint <dsh-root> [scan-root]  check CSS against it
 */
import { readFileSync, writeFileSync, readdirSync, existsSync } from 'node:fs'
import { join, relative, sep } from 'node:path'
import { parseTokens, lintCss, summarize, renderDeclaration } from './tokens.mjs'
...
if (command === undefined || dshRoot === undefined) {
  console.error('usage: node arbitration/tokens-cli.mjs <emit|lint> <dsh-root> [out.d.ts | scan-root]')
  process.exit(2)
}
